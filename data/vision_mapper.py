...
def check_extension(id,folder=str('G:/Giordano/downstream_datasets/YouCook2/videos')):
    # Cache the directory listing: GRAM runs this per video *read*; globbing the
    # whole folder for each frame sample makes >10k-video training O(N^2).
    cached = getattr(check_extension, '_dir_cache', {})
    if folder not in cached:
        cached[folder] = glob.glob(os.path.join(folder, '*'))
        check_extension._dir_cache = cached
    files_in_folder = cached[folder]
